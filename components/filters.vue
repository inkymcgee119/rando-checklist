<template>
    <div class="bg-slate-200 rounded-md shadow-xl ml-1 mr-1 pb-2">
        ...
        <div class="flex flex-row">
            <!-- settings -->
            <div v-if="settings && Object.keys(settings).length > 0" class="mx-2 my-auto">
            ...
            <div v-if="settings && Object.keys(settings).length > 0" class="grow">
                <span v-for="opt of Object.keys(settings)">
                    <button v-if="!settings[opt].filterWith && !settings[opt].isHidden"
                        ...
                        @click="clickSetting(settings[opt].name)">
            ...
            <!-- tags -->
            <div v-if="tags && Object.keys(tags).length > 0" class="mx-2 my-auto">
            ...
            <!-- toggle settings -->
            <div v-if="toggleSettings && Object.keys(toggleSettings).length > 0" class="mx-2 my-auto">
                <label class="font-sans font-semibold">{{ toggleSettingsDescription }}</label>
            </div>
            <div v-if="toggleSettings && Object.keys(toggleSettings).length > 0" class="grow my-auto">
                <span v-for="key of Object.keys(toggleSettings)" class="font-sans font-semibold mx-1 whitespace-nowrap">
                    <span v-if="!toggleSettings[key].isHidden">{{ toggleSettings[key].description }}
                        <toggle-switch v-model="modelValue.toggleSettings[key]" @updated="updateToggleSetting"></toggle-switch>
                    </span>
                </span>
            </div>

            <!-- toggle tags -->
            <div v-if="toggleTags && Object.keys(toggleTags).length > 0" class="mx-2 my-auto">
                <label class="font-sans font-semibold">{{ toggleTagsDescription }}</label>
            </div>
            <div v-if="toggleTags && Object.keys(toggleTags).length > 0" class="grow my-auto">
                <span v-for="key of Object.keys(toggleTags)" class="font-sans font-semibold mx-1 whitespace-nowrap">
...
function clickSetting(name) {
    props.modelValue.settings[name] = !props.modelValue.settings[name];
...
function updateToggleSetting(oldVal, newVal) {
    save();
}
